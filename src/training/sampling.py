...
def inference_loop_partial(
    ...
    if config.n_thinning == 1:
        ...
        def _inference_loop_partial(
            ...
            def one_step(state: State, xs: tuple[jnp.ndarray, jnp.ndarray]):
                ...
                state, info = sampler.step(rng_key, state)
                # dump y to disk
                # use function save_position_partial instead of save_position to 
                # change the intermediate layers with Gaussian prior
                jax.experimental.io_callback(
                    partial(save_position_partial, base=saving_path),
                    result_shape_dtypes=state.position,
                    position=state.position,
                    idx=step_id,
                    n=idx,
    ...
    if n_devices > 1:
        runner = jax.pmap(
            _inference_loop_partial,
            in_axes=(0, 0, 0, 0),
        )
        _rng = jax.random.split(sample_key, n_devices)
    else:
        runner = _inference_loop_partial
        _rng = sample_key
