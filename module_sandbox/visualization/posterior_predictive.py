...
def single_traceplot(samples: jnp.ndarray, id: str, path: str):
    """Plot the traceplot of a single parameter."""
    fig, ax = plt.subplots()
    for i in range(samples.shape[0]):
        ax.plot(samples[i, :])
    plt.savefig(f'{path}/traceplot{id}.png')
    plt.close()
...
def visualize_pp_chain_means(
    chain_means: jnp.ndarray, n_data_points: int = 100, show: bool = True
):
    """
