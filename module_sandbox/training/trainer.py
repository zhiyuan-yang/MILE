...
import module_sandbox.inference.metrics as sandbox_metrics
import module_sandbox.training.utils as train_utils
from module_sandbox.config.core import Config
from module_sandbox.config.data import DatasetType, Task
from module_sandbox.dataset import (
    ImageLoader,
    TabularLoader,
    TextLoader,
)
from module_sandbox.dataset.utils import CustomBPETokenizer
from module_sandbox.inference.metrics import (
    ClassificationMetrics,
    MetricsStore,
    RegressionMetrics,
)
from module_sandbox.inference.reporting import generate_html_report
from module_sandbox.training.probabilistic import ProbabilisticModel
from module_sandbox.training.sampling import inference_loop
from module_sandbox.types import ParamTree
from module_sandbox.utils import measure_time, pretty_string_dict
...
class BDETrainer:
    ...
    @property
    def batch_size_s(self):
        """Return the batch size for sampling."""
        return self.config_sampler.batch_size or len(self.loader.data_train)

    @property
    def n_batches(self):
        """Return the number of batches."""
        return (len(self.loader.data_train) // self.batch_size_s) or 1
    ...
    # module_sandbox/inference/inference.ipynb for extracting the time from the logs.
